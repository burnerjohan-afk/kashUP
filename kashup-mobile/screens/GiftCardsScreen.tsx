...
const MACARON_COLORS_CARTE = [
  { id: 'primary', label: 'Vert', value: '#05A357' },
...
export default function GiftCardsScreen() {
  ...
  const [personalSendMode, setPersonalSendMode] = useState<'email' | 'notification'>('email');
...
const [selectedPredefinedGift, setSelectedPredefinedGift] = useState<GiftCardOffer | null>(null);
  ...
  const [predefinedTheme, setPredefinedTheme] = useState(CARD_THEMES[0].id);
...
const [contentError, setContentError] = useState<string | null>(null);
  ...
  const filteredPartners = useMemo(() => {
    return partnerOptions.filter((partner) => {
      ...
      const matchesDept = partnerDeptFilter === 'all' || partner.country === partnerDeptFilter;
  ...
  const handlePersonalPurchase = async () => {
    if (!personalAmount || !personalRecipient) {
      Alert.alert('Informations manquantes', 'Merci de renseigner le montant et le bénéficiaire.');
      return;
    }
    ...
    if (personalSendMode === 'email' && !personalContact) {
      Alert.alert('Adresse e-mail requise', 'Ajoutez une adresse e-mail pour envoyer le bon.');
      return;
    }

    const giftCardId = resolveGiftCardId(personalPartnerId) ?? catalog[0]?.id;
    if (!giftCardId) {
      Alert.alert('Catalogue indisponible', 'Aucun bon à acheter pour le moment.');
      return;
    }

    if (personalSendMode === 'notification') {
      Alert.alert(
        'Mode notification',
        'L’envoi via notification arrivera bientôt. Utilisez le mode e-mail pour créer le bon.'
      );
      return;
    }

    const macaronText = selectionMacaron || selectionMacaronLibre;
    const messageForApi = macaronText
    ...
    try {
      await purchase({
        giftCardId,
        beneficiaryEmail: personalContact,
        amount: amountValue,
        message: messageForApi,
      });
      Alert.alert(
        'Carte envoyée',
        `Carte de ${formatCurrency(amountValue)} chez ${personalPartner?.name ?? 'KashUP'} envoyée à ${personalContact}.`
      );
      addNotification({
        category: 'cashback',
        title: 'Bon créé',
        description: `${personalPartner?.name ?? 'Partenaire KashUP'} • ${formatCurrency(amountValue)}`,
      });
  ...
  const openPredefinedModal = (gift: GiftCardOffer) => {
    ...
    setPredefinedTheme(CARD_THEMES[0].id);
    setPredefinedModalVisible(true);
  };

  const confirmPredefinedGift = () => {
    ...
    if (predefinedSendMode === 'email') {
      ...
      const subject = `Cadeau ${selectedPredefinedGift.partner.name} - ${selectedPredefinedGift.title}`;
      ...
      Alert.alert('Notification programmée', 'Le cadeau sera envoyé via notification KashUP.');
    }
                ...
                {(selectionMacaron || selectionMacaronLibre) ? (
                  <View
                    style={[
                      ...
                      {
                        backgroundColor: MACARON_COLORS_CARTE.find((c) => c.id === selectionMacaronColorId)?.value ?? '#05A357',
                      },
                    ]}>
  ...
  const renderFormFields = () => (
    <>
          ...
          {TEXT_COLORS_CARTE.map((c) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[styles.colorSwatch, { backgroundColor: c.value }, active && styles.colorSwatchActive]}
              />
            );
          })}
        </View>
      ...
      <View style={styles.sendModeRow}>
        <TouchableOpacity
          style={[styles.sendModeCard, personalSendMode === 'email' && styles.sendModeCardActive]}
          onPress={() => setPersonalSendMode('email')}
          activeOpacity={0.8}>
          <View style={[styles.sendModeIconWrap, personalSendMode === 'email' && styles.sendModeIconWrapActive]}>
            <Ionicons
              name="mail-outline"
              size={24}
              color={personalSendMode === 'email' ? colors.white : colors.primaryPurple}
            />
          </View>
          <Text style={[styles.sendModeTitle, personalSendMode === 'email' && styles.sendModeTitleActive]}>E-mail</Text>
          <Text style={styles.sendModeSubtitle}>Envoi par courriel</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.sendModeCard, personalSendMode === 'notification' && styles.sendModeCardActive]}
          onPress={() => setPersonalSendMode('notification')}
          activeOpacity={0.8}>
          <View style={[styles.sendModeIconWrap, personalSendMode === 'notification' && styles.sendModeIconWrapActive]}>
            <Ionicons
              name="notifications-outline"
              size={24}
              color={personalSendMode === 'notification' ? colors.white : colors.primaryPurple}
            />
          </View>
          <Text style={[styles.sendModeTitle, personalSendMode === 'notification' && styles.sendModeTitleActive]}>
            Notification KashUP
          </Text>
      ...
      <Text style={styles.sectionLabel}>
        {personalSendMode === 'email' ? 'Adresse e-mail' : 'Identifiant KashUP'}
      </Text>
      <TextInput
        style={styles.input}
        placeholder={personalSendMode === 'email' ? 'exemple@mail.com' : '@pseudo-kashup'}
        value={personalContact}
        onChangeText={setPersonalContact}
      />
      <View style={styles.formButtons}>
        <TouchableOpacity
          style={[styles.primaryButton, purchasing && styles.primaryButtonDisabled]}
          onPress={handlePersonalPurchase}
          disabled={purchasing}>
          <LinearGradient
            colors={[colors.primaryBlue, colors.primaryPurple]}
            start={{ x: 0, y: 0 }}
            ...
            <Text style={styles.primaryText}>
              {purchasing ? 'Envoi en cours…' : 'Envoyer la carte'}
            </Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>
            ...
            <>
              ...
              {openCommentCaMarche && (
                <Text style={[styles.offerSubtitle, { marginTop: spacing.xs }]}>
                  ...
                  4) Prévisualisez le rendu avec la carte test, puis indiquez les coordonnées du bénéficiaire et envoyez la carte par e-mail.
                </Text>
              )}
              </View>
            </>
                ...
                {(selectionMacaron || selectionMacaronLibre) ? (
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          MACARON_COLORS_CARTE.find((c) => c.id === selectionMacaronColorId)?.value ?? '#05A357',
                      },
                    ]}>
                    <Text style={styles.carteTestMacaronText} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>
                      {selectionMacaron === '' ? (selectionMacaronLibre.trim() || 'Votre texte') : selectionMacaron}
      ...
      {contentLoading ? (
        ...
        boxTemplates.map((box) => (
          <View key={box.id} style={styles.boxCard}>
            <ImageBackground
              source={{ uri: box.heroImageUrl ?? box.imageUrl ?? fallbackHeroImage }}
              style={styles.boxImage}
              imageStyle={styles.boxImageRadius}>
              <LinearGradient
                colors={['rgba(18,194,233,0.85)', 'rgba(164,69,255,0.85)']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
    ...
    <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
      ...
      >
        <View style={styles.pageHeader}>
          <Text style={styles.title}>Cartes UP</Text>
          <Text style={styles.subtitle}>
            Gestion des cartes cadeaux et boxes (aligné back office).
          </Text>
        </View>
        {contentError && (
      ...
      <PredefinedGiftModal
        ...
        onThemeChange={setPredefinedTheme}
...
function PredefinedGiftCard({ gift, onOffer }: PredefinedGiftCardProps) {
  const accent = gift.accentColor ?? getAccentColor(gift.id);
  const partnerName = gift.partner?.name ?? 'KashUP';
  const partnerLogo = gift.partner?.logoUrl;
  return (
    <View style={styles.predefinedCard}>
      <View style={[styles.predefinedLogoWrapper, { backgroundColor: `${accent}33` }]}>
        {partnerLogo ? (
          <Image source={{ uri: partnerLogo }} style={styles.predefinedLogo} resizeMode="contain" />
        ) : (
      ...
      <TouchableOpacity style={[styles.predefinedButton, { backgroundColor: accent }]} onPress={onOffer}>
...
function PredefinedGiftModal({
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={styles.modalSafeArea}>
        <ScrollView contentContainerStyle={styles.offerModal}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Offrir “{gift.title}”</Text>
            <TouchableOpacity onPress={onClose}>
              <Ionicons name="close" size={24} color={colors.textMain} />
            </TouchableOpacity>
          </View>
          <View style={styles.giftSummary}>
            {gift.partner?.logoUrl ? (
              <Image source={{ uri: gift.partner.logoUrl }} style={styles.summaryLogo} resizeMode="contain" />
            ) : (
          ...
          <View style={styles.radioRow}>
            {['email', 'notification'].map((mode) => {
              const active = sendMode === mode;
              return (
                <TouchableOpacity
                  key={mode}
                  style={[styles.radioChip, active && styles.radioChipActive]}
                  onPress={() => onSendModeChange(mode as 'email' | 'notification')}>
                  <Ionicons
                    name={active ? 'radio-button-on' : 'radio-button-off'}
                    size={18}
                    color={active ? colors.primaryBlue : colors.textSecondary}
                  />
                  <Text style={[styles.radioChipText, active && styles.radioChipTextActive]}>
                    {mode === 'email' ? 'E-mail' : 'Notification KashUP'}
                  </Text>
                </TouchableOpacity>
              );
            })}
          ...
          <Text style={styles.sectionLabel}>
            {sendMode === 'email' ? 'Adresse e-mail' : 'Utilisateur KashUP'}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={sendMode === 'email' ? 'exemple@mail.com' : '@pseudo-kashup'}
            value={recipient}
          ...
          <TextInput
            ...
            multiline
          />
          <Text style={styles.sectionLabel}>Visuel de la carte</Text>
              ...
              return (
                <TouchableOpacity
                  ...
                  <LinearGradient
                    ...
                    <Text style={styles.templateLabel}>{theme.label}</Text>
                  </LinearGradient>
                </TouchableOpacity>
              );
            })}
