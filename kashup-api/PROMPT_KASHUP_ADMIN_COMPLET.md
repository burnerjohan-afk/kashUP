# 🔧 PROMPT COMPLET POUR KASHUP-ADMIN
...
Dans le projet `kashup-admin`, le fichier `.env` doit être à la racine du projet, au même niveau que `package.json`.

### 1.2. Contenu du fichier `.env`
...
**Important :** Dans Vite, les variables d'environnement doivent commencer par `VITE_` pour être accessibles côté client.

---

## 📁 ÉTAPE 2 : Vérifier le code qui utilise l'API

### 2.1. Fichier à vérifier : Code qui fait les requêtes API
...
## 📁 ÉTAPE 3 : Gestion des erreurs API (IMPORTANT)
...
export async function apiRequest(endpoint: string, options: RequestInit = {}) {
  ...
  // L'API retourne toujours { statusCode, success, message, data, meta }
  if (!data.success) {
...
## 📁 ÉTAPE 4 : Gestion des requêtes multipart/form-data
...
Après modification du `.env`, redémarrez le serveur :
...
- **Format de réponse** : L'API retourne toujours `{ statusCode, success, message, data, meta }`
- **Gestion d'erreurs** : Toujours vérifier `data.success` dans les réponses
...
Avant d'utiliser kashup-admin, vérifiez :

- [ ] Le fichier `.env` est créé avec `VITE_API_URL=http://localhost:4000`
- [ ] `kashup-api` est démarré et accessible sur `http://localhost:4000`
- [ ] Le serveur de développement est redémarré après modification du `.env`
- [ ] Pas d'erreur CORS dans la console du navigateur
- [ ] Les requêtes API utilisent `import.meta.env.VITE_API_URL`
- [ ] Les requêtes avec `FormData` ne définissent PAS `Content-Type` dans les headers
- [ ] Les réponses API sont bien parsées en JSON
